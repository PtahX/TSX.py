...
def yahooKeyStats(stock): #Function(Parameters)
    try:
                    ...
                    if float(dToE) < 100:
                        ...
                        if float(priceSales) < 1:
                            ...
                            if currentPrice < grahamNum:
                                ...
                                sys.stdout = open('A:/_Investing/tsx.txt','w') # This Line Should Print and overwrite tsx.txt
                                print('*')
...
def scrapeTSX():  
    for eachStock in tsxFull:  # Assigns eachstock as each listed in tsxFull then runs it through yahookey stats
        yahooKeyStats(eachStock)
        time.sleep(2)

def scrapeNYSE():  
    for eachStock in NYSE:  # Assigns eachstock as each listed in tsxFull then runs it through yahookey stats
        yahooKeyStats(eachStock)
        time.sleep(2)

def scrapeTSX500():  
    for eachStock in tsx500:  # Assigns eachstock as each listed in tsxFull then runs it through yahookey stats
        yahooKeyStats(eachStock)
        time.sleep(2)
        #############################
    
def showTEST():
    print('Testing works woo hoo')
